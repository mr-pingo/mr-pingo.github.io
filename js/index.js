...
$(document).ready(function($) {
  var i=0;
  var anim = setInterval(function fadeI(){
    if(i<$(".message span").length){
       $(".message span").eq(i).animate({opacity:"1"},600);
    i++;
  }
  else{
    clearInterval(anim);
    $(".happy").animate({opacity:"1"},600);
    $(".press").delay(300).fadeIn(600);
    $(".press_mes").delay(400).animate({opacity:"1"},600);
    $(".inline-blocks").css("cursor","pointer").on("click",fadeO);
  }
  },500);
  function fadeO(){
  		$(".overlay-static").fadeOut(600);
  		$(".video").delay(600).fadeIn(600);
  	}
